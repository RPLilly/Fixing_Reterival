...
def _sanitize_tsquery(user_query: str) -> str:
    ...
    escaped = []
    for term in terms:
        safe = "".join(character for character in term if character.isalnum() or character == "_")
        if safe:
...
def main():
    ...
    questions_file = "single_chunk_questions.json"
    ...
    results_file = "retrieval_test_results_OR_operator_single_chunk.json"
